...
WINDSC = 20
DATASET = "AMIGOS" # AMIGOS, KEmoCon
DISDT = False
DIM = "Arousal"  # "Valence", Arousal
ANN_TH = 0
...
a = [1 if d > 0 else 0 for d in data["y_a"]]
print(np.unique(a, return_counts=True)[1]/len(a))
print(len(a))

_c = bp.signals.tools.pearson_correlation(data["y_a"], data["y_v"])[0]  # norm ??
...
for trainUS, testUS in kf.split(np.arange(len(Y))):  # divide by user  
    ...
    for timestmp in range(len(y_test)):  # iterate over timestamp for group dt
        ...
        for _usH in train_s:  # subjects in train set
            ...
            _c = np.nan_to_num(_c)
            if _c < 0:  # attention no non-weighted
