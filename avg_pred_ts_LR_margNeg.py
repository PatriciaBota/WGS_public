...
if DATASET == "AMIGOS":
    ...
    _dirres ="../Results/" + DATASET + "/RR_FV_A/"
...
TYPE = "FV"  # FV, IMG, MORP
inputX = data['RR_reddstfv'] #"EDA_redstfv" RR_redstfv, RR_spect
DIM = "Arousal"  # Arousal, Valence
...
for trainUS, testUS in kf.split(np.arange(len(Y))):  # divide by user  
    ...
    y_pred, wstd, eqLb, noCrr, _y_test, NegC = [], [], [], [], [], 0
    ...
    for timestmp in range(len(y_test)):  # iterate over timestamp for group dt
        ...
        for _usH in train_s:  # subjects in train set
            ...
            if _c < -0.05:
                NegC += 1
            if (_c < 0.05 and _c > -0.05):  # give opposite label
                _gp_l += [-y_train[train_usIDX][timestmp]] # subj ground truth  
                print(">>>>>", _c, y_train[train_usIDX][timestmp], y_test[timestmp])
                _c = np.abs(_c)
            else:
...
print("Perc Neg Corr", np.round(np.mean(perNegC), 2), " +- ", np.round(np.std(perNegC), 2))
print("\n")
